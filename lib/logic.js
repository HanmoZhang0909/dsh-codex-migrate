...
export const DEFAULTS = {
  ...
  // Tool output written into the DSH session, independent of the (longer)
  // archive copy. A thread carries thousands of tool results and they dominate
  // how much model context the imported conversation costs, so this is the knob
  // to keep an import continuable. 0 = use `maxToolOutputChars`.
  dshToolOutputChars: 0,
  maxDshTurns: 200,
...
const NUMBER_KEYS = ['bridgePort', 'syncIntervalMinutes', 'maxMessagesPerSession', 'maxToolOutputChars', 'maxProjectFileBytes', 'maxProjectFilesPerSync', 'dshMessageMaxChars', 'dshToolOutputChars', 'maxDshTurns', 'keepLatestTurns']
...
export function createLogic(ctx, rowConfig, home) {
  ...
  function buildDshEvents(codexId, blocks, title, result) {
    ...
    const toolOutputMaxChars = Number.isFinite(config.dshToolOutputChars) && config.dshToolOutputChars > 0
      ? config.dshToolOutputChars
      : (Number.isFinite(config.maxToolOutputChars) && config.maxToolOutputChars > 0 ? config.maxToolOutputChars : maxChars)
    const pendingCalls = {}
    ...
    for (let i = 0; i < blocks.length; i++) {
      ...
      if (block.kind === 'user') {
        ...
        ensureTurn()
        // One assistant message carries this group's tool calls, and the chat
        // protocol requires every call id it announces to be answered by tool
        // messages that follow it. Codex records interrupted calls without an
        // output, and a missing or mismatched id makes the whole conversation
        // unusable ("An assistant message with 'tool_calls' must be followed by
        // tool messages responding to each 'tool_call_id'"), so calls and
        // results are paired explicitly and every call is answered.
        let groupEnd = i
        while (groupEnd < blocks.length && (blocks[groupEnd].kind === 'tool' || blocks[groupEnd].kind === 'toolOut')) groupEnd++
        const calls = []
        let fallback = 0
        for (let k = i; k < groupEnd; k++) {
          ...
          if (b.kind === 'tool') {
            fallback++
            const callId = typeof b.callId === 'string' && b.callId !== '' ? b.callId : ('cxcall-' + String(fallback) + '-' + String(k))
            calls.push({ id: callId, block: b, result: null })
          } else if (b.kind === 'toolOut') {
            const wanted = typeof b.callId === 'string' && b.callId !== '' ? b.callId : ''
            const target = (wanted === '' ? undefined : calls.find((call) => call.id === wanted && call.result === null))
              ?? calls.find((call) => call.result === null)
            if (target === undefined) {
              // A result without a call cannot be expressed in the protocol.
              if (result.dsh.notes.length < 10) result.dsh.notes.push(L('noteOrphanToolResult', { id: wanted }))
              continue
            }
            target.result = b
          }
        ...
        const toolContent = calls.map((call) => ({
          type: 'tool-call',
          id: call.id,
          name: call.block.name || 'tool',
          arguments: typeof call.block.args === 'string' ? call.block.args : '{}',
        }))
        push('assistant/message', {
          turn,
          step: nextStep(),
        ...
        for (const call of calls) {
          const st = nextStep()
          steps.set(call.id, st)
          pendingCalls[call.id] = { turn, step: st }
          lastToolLoc = { turn, step: st }
          push('tool/call', { turn, step: st, callId: call.id, name: call.block.name || 'tool', arguments: typeof call.block.args === 'string' ? call.block.args : '{}' })
        }
        for (const call of calls) {
          const st = steps.get(call.id)
          // Tool output is capped by its own setting: a long thread carries
          // thousands of results, and they dominate how much context the
          // imported conversation costs (`maxToolOutputChars` used to be
          // honoured only while parsing, never when emitting the events).
          const text = call.result === null ? L('toolNoResult') : truncate(call.result.text, toolOutputMaxChars)
          msgN++
          push('tool/result', {
            turn,
            step: st,
            message: {
              id: 'msg-t' + msgN,
              role: 'user',
              content: [{ type: 'tool-result', toolCallId: call.id, content: [{ type: 'text', text }] }],
  ...
  /**
   * The working directory of a stored DSH session. rc.1 `load`/`inspect`
   * answered with `meta`, the rc.2 handle seam answers with `header`, and
   * reading only one of the two silently loses the workspace — a DSH -> Codex
   * export then lands in the harness home instead of the project.
   */
  function dshProjectKey(loaded) {
    return String(loaded?.meta?.cwd ?? loaded?.header?.cwd ?? '')
  }
  ...
  async function readDshConversation(conversationId) {
    ...
    return {
      id: conversationId,
      projectKey: dshProjectKey(loaded),
  ...
  async function authoritativeDshContext(conversationId) {
    ...
    const projectKey = dshProjectKey(currentLoaded)
