...
const NUMBER_KEYS = ['bridgePort', 'syncIntervalMinutes', 'maxMessagesPerSession', 'maxToolOutputChars', 'maxProjectFileBytes', 'maxProjectFilesPerSync', 'dshMessageMaxChars', 'maxDshTurns']
...
export function createLogic(ctx, rowConfig, home) {
  ...
  async function inspectDshConversation(conversationId) {
    ...
    if (typeof persistence.inspect === 'function') return persistence.inspect(conversationId)
    if (typeof persistence.load === 'function') return persistence.load(conversationId)
    throw new Error('DSH session persistence cannot inspect conversations')
  }
    ...
    if (!dshConversationAdapter) {
      dshConversationAdapter = Object.freeze({
        ...
        async createCounterpart(seed) {
          ...
          await persistence.create({ version: 0, id: conversationId, createdAt: Date.now(), cwd })
          await persistence.append(conversationId, events)
        ...
        async appendTurns(conversationId, turns) {
          ...
          if (events.length > 0) await persistence.append(conversationId, events)
